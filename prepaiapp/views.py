    ...
    def get(self, request, share_id):
        ...
        if self.request.user.is_authenticated:
            MyInvitedRolePlayShare.objects.get_or_create(
                share=share, bot=bot, invited_to=request.user
            )
        return render(request, "roleplay_bot_detail.html", context)
        ...
        try:
            ...
            if avatar_file:
                ...
                if avatar_file.content_type not in allowed_types:
                    messages.error(
                        request, "Avatar must be a JPEG, PNG, or WebP image."
                    )
                    return redirect("edit-roleplay-bot", bot_id=bot_id)
                ...
                if not avatar_url:
                    messages.error(
                        request, "Failed to upload avatar image. Please try again."
                    )
                    return redirect("edit-roleplay-bot", bot_id=bot_id)
            ...
            if len(system_prompt) < 50:
                messages.error(
                    request,
                    "System prompt should be more detailed (at least 50 characters).",
                )
                return render(request, "edit_roleplay_bot.html", {"bot": bot})
            ...
            if custom_configuration:
                try:
                    ...
                    messages.error(
                        request, "Custom configuration must be valid JSON format."
                    )
                    return render(request, "edit_roleplay_bot.html", {"bot": bot})
            ...
            messages.error(
                request, f"An error occurred while updating the bot: {str(e)}"
            )
            return render(request, "edit_roleplay_bot.html", {"bot": bot})
...
class DeleteRolePlayBotView(LoginRequiredMixin, View):
    ...
    def post(self, request, bot_id):
        ...
        try:
            ...
            messages.error(
                request, f"An error occurred while deleting the bot: {str(e)}"
            )
...
class MyRolePlayBotView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        roleplay_bots = RolePlayBots.objects.filter(created_by=request.user).order_by(
            "order", "-created_at"
        )
...
class CreateRolePlayBotView(LoginRequiredMixin, View):
    ...
    def post(self, request):
        ...
        if profile.credits < credit_required_to_create_bot:
            messages.error(
                request, f"Low credits- Required credit {credit_required_to_create_bot}"
            )
            return redirect("purchase_credits")
        ...
        custom_configuration = {
            ...
            "required_minimum_credits": int(
                request.POST.get("required_minimum_credits", 10)
            ),
        }
        ...
        try:
            ...
            messages.success(
                request, f"Roleplay Bot '{bot.name}' created successfully!"
            )
            ...
            messages.error(
                request, "An error occurred while creating the bot. Please try again."
            )
            return redirect("create_roleplay_bot")
    ...
    def get(self, request, session_id):
        try:
            ...
            session = get_object_or_404(
                RoleplaySession, id=session_id, user=request.user
            )
            share_link = MyInvitedRolePlayShare.objects.filter(
                bot=session.bot, invited_to=request.user
            ).first()
            context = {
                "session": session,
                "bot": session.bot,
...
class RolePlayStartView(LoginRequiredMixin, View):
    ...
    def get(self, request, bot_id):
        try:
            ...
            role_play_bot = get_object_or_404(RolePlayBots, id=bot_id, is_active=True)
            profile = Profile.objects.get(user=request.user)
            if not profile.has_credit(
                required=role_play_bot.custom_configuration.get(
                    "required_minimum_credits", 10
                )
            ):
            ...
            ongoing_session = RoleplaySession.objects.filter(
                user=request.user, bot=role_play_bot, status="in_progress"
            ).first()
            ...
            profile.deduct_credits(
                used=role_play_bot.custom_configuration.get(
                    "required_minimum_credits", 10
                )
            )
            ...
            messages.error(
                request, "Failed to start Roleplay session. Please try again."
            )
            return redirect("voice_roleplay")
...
class VoiceRolePlayView(View):
    ...
    def get(self, request):
        ...
        if selected_category == "professional":
            career_bots = all_bots.filter(category__in=career_categories).order_by(
                "order", "-created_at"
            )
            learning_bots = []
            personal_bots = []
            ...
            career_bots = []
            personal_bots = []
            learning_bots = all_bots.filter(category__in=learning_categories).order_by(
                "order", "-created_at"
            )
            entertainment_bots = []
            ...
            personal_bots = []
            learning_bots = []
            entertainment_bots = all_bots.filter(
                category__in=entertainment_categories
            ).order_by("order", "-created_at")
        elif selected_category == "personal":
            personal_bots = all_bots.filter(category__in=personal_categories).order_by(
                "order", "-created_at"
            )
            career_bots = []
            learning_bots = []
...
class PurchaseCredits(View):
    ...
    def get(self, request):
        ...
        country_code = self.get_country_from_ip(user_ip)
        ...
        if is_indian:
            ...
            currency_symbol = "₹"
            plans = {
                "mini": {"credits": 40, "price": 49, "minutes": 4, "rate": "12.25"},
                "growth": {"credits": 300, "price": 249, "minutes": 30, "rate": "8.3"},
                "pro": {"credits": 1500, "price": 999, "minutes": 150, "rate": "6.6"},
            }
        else:
            currency = "USD"
            currency_symbol = "$"
            plans = {
                "mini": {"credits": 100, "price": 2.99, "minutes": 10, "rate": "0.30"},
                "growth": {
                    "credits": 500,
                    "price": 12.99,
                    "minutes": 50,
...
class InterviewHistoryView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        base_sessions = InterviewSession.objects.filter(
            user=request.user
    ...
    def calculate_statistics(self, sessions):
        ...
        # Most attempted role
        role_stats = (
            sessions.values("template__role_type")
    ...
    def get(self, request, session_id):
        try:
            ...
            if not analysis_results:
                analysis_results = (
                    json.loads(session.feedback) if session.feedback else None
                )
            ...
            if not analysis_results:
                messages.error(
                    request, "Unable to analyze interview results. Please try again."
                )
                return redirect("interview_types")
            ...
            context = {
                ...
                # Skills assessment
                "skills_assessment": self.format_skills_assessment(
                    analysis_results.get("skills_assessment", [])
                ),
                # Feedback sections
                "strengths": self.format_feedback_items(
                    analysis_results.get("detailed_feedback", {}).get("strengths", [])
                ),
                "improvements": self.format_feedback_items(
                    analysis_results.get("detailed_feedback", {}).get(
                        "areas_for_improvement", []
                    )
                ),
                "recommendations": self.format_feedback_items(
                    analysis_results.get("detailed_feedback", {}).get(
                        "recommendations", []
                    )
                ),
                # Statistics
                "total_questions": analysis_results.get("statistics", {}).get(
                    "questions_asked", 0
                ),
                "total_responses": analysis_results.get("statistics", {}).get(
                    "candidate_responses", 0
                ),
                "words_spoken": analysis_results.get("statistics", {}).get(
                    "words_spoken", 0
                ),
                "avg_response_time": self.extract_response_time_seconds(
                    analysis_results.get("statistics", {}).get(
    ...
    def format_feedback_items(self, feedback_list):
        ...
        for i, feedback in enumerate(feedback_list, 1):
            if isinstance(feedback, str):
                ...
                formatted_items.append({"title": f"Point {i}", "description": feedback})
            ...
            if line.startswith("[") and "]" in line:
                ...
                try:
                    ...
                    if ":" in remainder:
                        ...
                        # Map speaker to role
                        role = (
                            "assistant" if "INTERVIEWER" in speaker.upper() else "user"
                        )
    ...
    def get(self, request, template_id):
        try:
            ...
            template = get_object_or_404(
                InterviewTemplate, id=template_id, is_active=True
            )
            profile = Profile.objects.get(user=request.user)
            ...
            ongoing_session = InterviewSession.objects.filter(
                user=request.user, template=template, status="in_progress"
            ).first()
            ...
            messages.error(
                request, "Failed to start interview session. Please try again."
            )
            return redirect("interview_types")
    ...
    def get(self, request, session_id):
        try:
            ...
            if session.status not in ["in_progress", "pending"]:
                messages.info(
                    request, "This interview session has already been completed."
                )
...
class CourseView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        courses_queryset = Course.objects.filter(is_active=True).order_by(
            "order", "title"
        )
        ...
        for course in courses_queryset:
            print(
                f"Course: {course.title} - Icon: {course.icon} - Category: {course.category}"
            )
...
class SignupView(View):
    ...
    def get(self, request):
        ...
        return render(
            request, "signup.html", {"next": next_url}
        ...
        if share_id:
            try:
                ...
                if share.shared_by != self.request.user:
                    MyInvitedRolePlayShare.objects.get_or_create(
                        share=share, bot=share.bot, invited_to=self.request.user
                    )
                if bot_id:
...
class LoginView(View):
    ...
    def get(self, request):
        ...
        if request.user.is_authenticated:
            ...
            return redirect("dashboard")
        return render(
            request, "login.html", {"next": next_url}
...
def health_check(request):
    ...
    return JsonResponse({"status": "healthy", "service": "voice-agent"})


def api_status(request):
    """API status endpoint"""
    return JsonResponse({"websocket_url": "/ws/voice-agent/", "status": "ready"})
...
class EarlyAccessSignupView(View):
    ...
    def post(self, request):
        ...
        if email:
            ...
            if created:
                return JsonResponse(
                    {"status": "success", "message": "Thanks for joining!"}
                )
            else:
                return JsonResponse(
                    {"status": "info", "message": "You're already on the list."}
                )
        else:
            return JsonResponse(
    ...
    def post(self, request):
        ...
        with transaction.atomic():
            for idx, bot_data in enumerate(bots_data):
                try:
                    # Validate required fields
                    if not bot_data.get("name") or not bot_data.get(
                        "system_prompt", "N/A"
                    ):
                        errors.append(
                            {
                    ...
                    created_bots.append(
                        {"id": str(bot.id), "name": bot.name, "category": bot.category}
                    )
        ...
        return Response(
            ...
            status=(
                status.HTTP_201_CREATED if created_bots else status.HTTP_400_BAD_REQUEST
            ),
        )
